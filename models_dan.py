...
class NETWORK_OF_DANS_batch(nn.Module):
    ...
    def __init__(self, dims, n_channels,genome=None,):
        ...
        self.Skip_layers["0_3"] = nn.Linear(self.dims[0], self.dims[3]*self.n_channels)
        ...
        for l in range(len(self.dims)-1):
            ...
            #     self.DAN_layers["layer_"+str(l)+"_node_"+str(node_idx)] = Simple_DAN(n_channels+len(self.layer_bias_codes[0][0]))
    ...
    def forward(self, x):
        ...
        for layer_key in self.VECS_layers:
            ...
            if layer_idx == 1:
                ...
                self.skip_acts_0_2 = self.Skip_layers["0_2"](x)
                self.skip_acts_0_3 = self.Skip_layers["0_3"](x)
                self.skip_acts_1_3 = self.Skip_layers["1_3"](hidden_acts_copy)
                hidden_acts = hidden_acts.add_(self.skip_acts_0_2)
                # hidden_acts = self.dropout1(hidden_acts)
            if layer_idx == 2:
                hidden_acts = hidden_acts.add_(self.skip_acts_0_3)
                hidden_acts = hidden_acts.add_(self.skip_acts_1_3)
                # hidden_acts = self.dropout2(hidden_acts)
            
            #hidden_acts = F.leaky_relu(hidden_acts)
...
###########################################################################################################
class NETWORK_OF_DANS_AE(nn.Module):

    def __init__(self, dims, n_channels,genome=None,):
        super(NETWORK_OF_DANS_AE, self).__init__()   
        self.dims = dims 
        self.num_DAN_layers = len(dims)-1
        self.n_channels = n_channels 
        ...
        self.DAN = Simple_DAN(n_channels)
        self.dan_acts = torch.Tensor().cpu()
        self.DAN_layers["conv1"] = nn.Conv2d(1,16,3,1)
        self.DAN_layers["conv2"] = nn.Conv2d(16, 32, 3, 1)
        #self.DAN_layers["conv3"] = nn.Conv2d(32, 1, 2, 1)
        self.DAN_layers["conv_out1"] = nn.Conv2d(1,16,3,1, padding=1)
        self.DAN_layers["conv_out2"] = nn.Conv2d(16,1,3,1, padding=1)
        self.DAN_layers["DAN_phenotype_1"] = self.DAN
        #self.DAN_layers["DAN_phenotype_2"] = Simple_DAN(n_channels)
       

        #self.DAN_layers["fc1"] = nn.Linear(self.dims[0], self.dims[1]*n_channels)
        # construct fully-connected layers of VECS -------------------------------------------
        for l in range(0,len(self.dims)-1):
            if l < 1:
                self.DAN_layers["layer_"+str(l)] = nn.Linear(self.dims[l], self.dims[l+1]*n_channels)
            else:
                self.VECS_layers["layer_"+str(l)] = nn.Linear(self.dims[l], self.dims[l+1]*n_channels)
    ...
    def forward(self, x):
        ...
        # first fully connected layer is meant to be fixed -----------------------------------------------------
        hidden_acts_copy = hidden_acts.clone().detach()  
        hidden_acts = self.DAN_layers["layer_0"](hidden_acts) 
        hidden_acts = torch.reshape(hidden_acts, (hidden_acts.size()[0],self.dims[1],self.n_channels))
        self.dan_acts = self.DAN_layers["DAN_phenotype_1"](hidden_acts)
        self.dan_acts = torch.reshape(self.dan_acts, (self.dan_acts.size()[0], self.dims[1]))  
        hidden_acts = self.dan_acts.view(self.dan_acts.size()[0], self.dan_acts.size()[1]) 
        # first fully connected layer is meant to be fixed -----------------------------------------------------
        ...
        for layer_key in self.VECS_layers:            
            ...
            hidden_acts = self.VECS_layers[layer_key](hidden_acts) 
            hidden_acts = torch.reshape(hidden_acts, (hidden_acts.size()[0],self.dims[layer_idx+1],self.n_channels))
        
            #if layer_idx == 0:
            self.dan_acts = self.DAN_layers["DAN_phenotype_1"](hidden_acts)
            # else:
            #     self.dan_acts = self.DAN_layers["DAN_phenotype_2"](hidden_acts)
            self.dan_acts = torch.reshape(self.dan_acts, (self.dan_acts.size()[0], self.dims[layer_idx+1]))  
            hidden_acts = self.dan_acts.view(self.dan_acts.size()[0], self.dan_acts.size()[1]) 
        

            layer_idx +=1
        
        hidden_acts = hidden_acts.view((hidden_acts.size()[0],1,28,28))
        hidden_acts = self.DAN_layers["conv_out1"](hidden_acts)
        hidden_acts = F.leaky_relu(hidden_acts)
        hidden_acts = self.DAN_layers["conv_out2"](hidden_acts)
        hidden_acts = F.leaky_relu(hidden_acts)
        # print(hidden_acts.size())
        # sys.exit()
...
###########################################################################################################
class NETWORK_OF_Conv_DANS_AE(nn.Module):

    def __init__(self, dims, n_channels):
        super(NETWORK_OF_Conv_DANS_AE, self).__init__()   
        self.dims = dims 
        self.num_DAN_layers = len(dims)-1
        self.n_channels = n_channels 
        self.VECS_layers = nn.ModuleDict() 
        self.DAN_layers = nn.ModuleDict() 
        self.ENC_layers = nn.ModuleDict() 
        self.DAN = Simple_DAN(n_channels)
        self.dan_acts = torch.Tensor().cpu()
        self.DAN_layers["conv1"] = nn.Conv2d(1,8,3,1, padding=1)
        self.DAN_layers["conv2"] = nn.Conv2d(8, 1, 3, 1, padding=1)
        self.ENC_layers["encoder1"] = nn.Linear(self.dims[0],self.dims[1]*n_channels)
        self.ENC_layers["encoder2"] = nn.Linear(self.dims[1],self.dims[2]*n_channels)
        self.VECS_layers["decoder1"] = nn.Linear(self.dims[2],self.dims[3]*n_channels)
        self.VECS_layers["decoder2"] = nn.Linear(self.dims[3],self.dims[4]*n_channels)
        self.DAN_layers["DAN_phenotype_1"] = self.DAN

        self.ENC_layers["skip_0_2"] = nn.Linear(self.dims[0],self.dims[2]*n_channels)
        self.VECS_layers["skip_2_4"] = nn.Linear(self.dims[2],self.dims[4]*n_channels)
        
       
    #    784(conv) -> 64(conv) -> 784(conv)
    ...
    def forward(self, x):
        ...
        x = self.ENC_layers["encoder1"](x)
        skip_0_2 = self.ENC_layers["skip_0_2"](skipx)
        
        x = x.view((x.size()[0],self.dims[1],self.n_channels))
        x = self.DAN_layers["DAN_phenotype_1"](x)
        x = torch.flatten(x,1)

        x = self.ENC_layers["encoder2"](x)
        x = x.add_(skip_0_2)

        x = x.view((x.size()[0],self.dims[2],self.n_channels))
        x = self.DAN_layers["DAN_phenotype_1"](x)
        x = torch.flatten(x,1)
        skipx = x

        x = self.VECS_layers["decoder1"](x)
        skip_2_4 = self.VECS_layers["skip_2_4"](skipx)
        
        x = x.view((x.size()[0],self.dims[3],self.n_channels))
        x = self.DAN_layers["DAN_phenotype_1"](x)
        x = torch.flatten(x,1)

        x = self.VECS_layers["decoder2"](x)
        x = x.add_(skip_2_4)

        x = x.view((x.size()[0],self.dims[4],self.n_channels))
        x = self.DAN_layers["DAN_phenotype_1"](x)
        x = torch.flatten(x,1)
...
###########################################################################################################
class NETWORK_OF_Conv_DANS_AE_v2(nn.Module):

    def __init__(self, dims, n_channels):
        super(NETWORK_OF_Conv_DANS_AE_v2, self).__init__()   
        self.dims = dims 
        self.num_DAN_layers = len(dims)-1
        self.n_channels = n_channels 
        self.VECS_layers = nn.ModuleDict() 
        ...
        self.ENC_layers["encoder1"] = nn.Linear(self.dims[0],self.dims[1]*n_channels)
        self.ENC_layers["encoder2"] = nn.Linear(self.dims[1],self.dims[2]*n_channels)
        self.ENC_layers["encoder3"] = nn.Linear(self.dims[2],self.dims[3]*n_channels)
        self.VECS_layers["decoder1"] = nn.Linear(self.dims[3],self.dims[4]*n_channels)
        self.VECS_layers["decoder2"] = nn.Linear(self.dims[4],self.dims[5]*n_channels)
        self.VECS_layers["decoder3"] = nn.Linear(self.dims[5],self.dims[6]*n_channels)
        self.DAN_layers["DAN_phenotype_1"] = self.DAN

        self.ENC_layers["skip_0_3"] = nn.Linear(self.dims[0],self.dims[3]*n_channels)
        self.VECS_layers["skip_3_6"] = nn.Linear(self.dims[3],self.dims[6]*n_channels)
        
       
    #    [784, 200, 100, 25, 100, 200, 784]
    ...
    def forward(self, x):
        ...
        x = self.ENC_layers["encoder1"](x)
        skip_0_3 = self.ENC_layers["skip_0_3"](skipx)
        ...
        x = self.VECS_layers["decoder1"](x)
        skip_3_6 = self.VECS_layers["skip_3_6"](skipx)
        ...
        x = x.view((x.size()[0],self.dims[5],self.n_channels))
        x = self.DAN_layers["DAN_phenotype_1"](x)
        x = torch.flatten(x,1)

        x = self.VECS_layers["decoder3"](x)
        x = x.add_(skip_3_6)

        x = x.view((x.size()[0],self.dims[6],self.n_channels))
        x = self.DAN_layers["DAN_phenotype_1"](x)
        x = torch.flatten(x,1)
...
###########################################################################################################
class CONV_DAN_AE(nn.Module):

    def __init__(self, dims, n_channels,genome=None,):
        super(CONV_DAN_AE, self).__init__()   
        self.dims = dims 
        self.num_DAN_layers = len(dims)-1
        self.n_channels = n_channels 
        self.VECS_layers = nn.ModuleDict() 
        ...
        self.dan_acts = torch.Tensor().cpu()
        self.DAN_layers["conv1"] = nn.Conv2d(1,32,3,1, padding=1)
        self.DAN_layers["conv2"] = nn.Conv2d(32, 1, 3, 1, padding=1)
        self.DAN_layers["encoder1"] = nn.Linear(784,225)
        self.DAN_layers["encoder2"] = nn.Linear(225,64)
        
        self.DAN_layers["decoder1"] = nn.Linear(64,225)
        self.VECS_layers["decoder2"] = nn.Linear(225, 784)
    ...
    def forward(self, x):
        ...
        #-- (encoder 2) VECS layer 2 ----------------------------
        x = self.DAN_layers["encoder2"](x)
        x = x.view((x.size()[0],1,8,8))
        #-- end (encoder 2) VECS layer 2 ------------------------
        ...
        hidden_acts = x 
        hidden_acts = hidden_acts.view((hidden_acts.size()[0],1,28,28))
        hidden_acts = self.DAN_layers["conv1"](hidden_acts)
        hidden_acts = F.leaky_relu(hidden_acts)
        hidden_acts = self.DAN_layers["conv2"](hidden_acts)
        hidden_acts = F.leaky_relu(hidden_acts)
        #-- end DAN layer 3 ----------------------------
...
###########################################################################################################
class CONV_DAN_AE_v2(nn.Module):

    def __init__(self, dims, n_channels,genome=None,):
        super(CONV_DAN_AE_v2, self).__init__()   
        self.dims = dims 
        self.num_DAN_layers = len(dims)-1
        self.n_channels = n_channels 
        self.VECS_layers = nn.ModuleDict() 
...
###########################################################################################################
class NETWORK_OF_DANS_conv(nn.Module):
    """
        params:
            dims (array of ints): array of layer widths.  Must be multiples of n_channels
            n_channels (int):  denotes input dimensions for DANs 
        ...
        TO-DO:
            - pass skip layers as parameter and automate in init & forward
    """
    def __init__(self, dims, n_channels, conv_params):
        super(NETWORK_OF_DANS_conv, self).__init__()   
        self.dims = dims 
        ...
        self.DAN = Simple_DAN(n_channels)
        self.dan_acts = torch.Tensor().cpu()
        self.Conv_params = nn.ModuleDict()
        self.Conv_params["conv0"] = Simple_DAN(self.conv_params[0]*self.conv_params[1])
        self.Conv_params["conv1"] = Simple_DAN(self.conv_params[0]*self.conv_params[1])
        self.Conv_params["conv2"] = Simple_DAN(self.conv_params[0]*self.conv_params[1])
    ...
    def forward(self, x):
        ...
        #do convolution layer first 
        # do i keep the data in 2d or flatten it, prob keep it in 2d, concat the outputs 
        self.conv_acts = torch.Tensor().cpu()
        for y_idx in range(x.shape[1]-self.conv_params[0]):
            for x_idx in range(x.shape[2]-self.conv_params[0]):
                slice = x[:,y_idx:y_idx+self.conv_params[0], x_idx:x_idx+self.conv_params[0]]
                slice = torch.reshape(slice, (slice.shape[0], self.conv_params[0]*self.conv_params[0]))
                for z in range(3):
                    self.conv_acts = torch.cat((self.conv_acts, self.Conv_params["conv"+str(z)](slice)), dim=1)
        # print("conv acts", self.conv_acts.shape)
        # sys.exit()
        hidden_acts = self.conv_acts
        ...
        for layer_key in self.VECS_layers:
            ...
            if layer_idx == 1:
                ...
                self.skip_acts_0_2 = self.Skip_layers["0_2"](self.conv_acts)
                self.skip_acts_0_3 = self.Skip_layers["0_3"](self.conv_acts)
                self.skip_acts_1_3 = self.Skip_layers["1_3"](hidden_acts_copy)
                hidden_acts = hidden_acts.add_(self.skip_acts_0_2)
                # hidden_acts = self.dropout1(hidden_acts)
            if layer_idx == 2:
